...
def extract_release_group(text: str) -> tuple[str, str | None]:
    """
    ...
    match = re.search(
        r"[-_](?:DVD[Rr]ip[-_]?)?(?:XviD|x264|AVC|MPEG4|DivX)[-_]?(\d{4})?[-_]?([a-zA-Z0-9_]+)?$", text, re.IGNORECASE
    )
    if match:
        if match.group(2):
    ...
    match = re.search(
        r"[-_]DVD[Rr]i[Pp][-_]XviD[-_](?:RE[Rr]i[Pp][-_])?(\d{4})[-_]([a-zA-Z0-9_]+)$", text, re.IGNORECASE
    )
    if match:
        release_group = match.group(2)
        text = text[: match.start()]
    ...
    text = re.sub(
        r"[-_.](?:SVCD|VCD|DVDRip|WEBRip|HDTV|PDTV|720p|1080p|AVC|MKV|x264|XviD|AC-3|AAC)[-_.]?(?:\d{4})?$",
        "",
        text,
        flags=re.IGNORECASE,
    )
...
def get_video_info(filepath: str) -> ProbeResult | None:
...
def parse_filename(filename: str) -> dict:
    ...
    result = {"artist": None, "title": None, "is_live_indicator": False, "pattern_matched": None, "release_group": None}
...
def print_results(results: list[VideoMetadata], output_format: str = "table", output_file: str = None):
    ...
    def print_section(title: str, items: list[VideoMetadata]):
        ...
        print("-" * 170)
        print(
            f"{'Artist':<25} {'Title':<32} {'Duration':<10} {'Resolution':<12} {'Video':<8} {'Audio':<8} {'Bitrate':<11} {'Size':<11} {'Date':<12}"
        )
        print("-" * 170)
        for item in items:
            ...
            print(
                f"{artist:<25} {title:<32} {item.duration_formatted:<10} {item.resolution:<12} {item.video_codec:<8} {item.audio_codec:<8} {item.bitrate:<11} {item.filesize:<11} {item.file_date:<12}"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Extract metadata from music video files")
    parser.add_argument(
        "directory", nargs="?", default=".", help="Directory containing music videos (default: current directory)"
    )
    parser.add_argument(
        "-f", "--format", choices=["table", "json", "csv"], default="table", help="Output format (default: table)"
    )
    parser.add_argument(
        "-o", "--output", type=str, help="Output file path (for csv/json). If not specified, prints to stdout"
    )
    parser.add_argument(
        "-t",
        "--threshold",
